import {
  for_each = var.import_existing ? local.required_services : toset([])
  to       = google_project_service.required[each.value]
  id       = "${var.project_id}/${each.value}"
}

import {
  for_each = var.import_existing ? local.service_accounts : {}
...
import {
  for_each = var.import_existing ? local.active_secrets : {}
  to       = google_secret_manager_secret.active[each.key]
  id       = "projects/${var.project_id}/secrets/${each.value.secret_id}"
}

import {
  for_each = var.import_existing ? toset(["existing"]) : toset([])
  to       = google_project_iam_binding.no_basic_editor
  id       = "${var.project_id} roles/editor"
}

import {
  for_each = var.import_existing ? local.legacy_bucket_bindings : {}
  to       = google_storage_bucket_iam_binding.no_legacy_access[each.key]
  id       = "b/${each.value.bucket} ${each.value.role}"
}
